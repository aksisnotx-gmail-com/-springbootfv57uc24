			...
			<el-form class="center-form-pv" :style='{"border":"0px solid rgba(0, 0, 0, 0.05)","padding":"20px 20px","margin":"0px","overflow":"hidden","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"linear-gradient(323deg, rgba(221,221,221,.5) 0%, rgba(255,255,255,.5) 70%, rgba(221,221,221,.5) 100%),url(http://codegen.caihongy.cn/20230809/02ecb2d595ed42a2b0e8af76771b2f48.gif)","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","color":"#333","textAlign":"left","background":"none","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">日程名称</label>
						<el-input v-model="searchForm.richengmingcheng" placeholder="日程名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0px","margin":"0px 0 0px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('richenganpai','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('richenganpai','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"20px","boxShadow":"0 2px 3px 0px rgba(100,100,100,.05)","borderColor":"rgba(252,234,238,1)","margin":"0 0 0px","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230809/02ecb2d595ed42a2b0e8af76771b2f48.gif)","borderWidth":"0px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'
						prop="richengmingcheng"
						label="日程名称">
						<template slot-scope="scope">
							{{scope.row.richengmingcheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
						prop="riqi"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('richenganpai','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('richenganpai','工作日志')" @click="gongzuorizhiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','已提交工作日志')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								工作日志
							</el-button>
							<el-button class="edit" v-if=" isAuth('richenganpai','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('richenganpai','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import gongzuorizhiCrossAddOrUpdate from "../gongzuorizhi/add-or-update";
	export default {
		...
		methods: {
			...
			gongzuorizhiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.gongzuorizhiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','richenganpai');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.gongzuorizhiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
    ...
    init () {
          let urlriqi = 'richenganpai/remind/riqi/2';
		 urlriqi+=`?remindend=0`,

            	urlriqi+=`&remindstart=0`,
          ...
          this.$http({
            ...
            if (data && data.code == 0 && data.count>0 ) {
              this.$alert(`日期有${data.count}条数据到期`)
            }
          });
    },
    search() {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.richengmingcheng!='' && this.searchForm.richengmingcheng!=undefined){
            params['richengmingcheng'] = '%' + this.searchForm.richengmingcheng + '%'
          }
	...
	richenganpaistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'richenganpai/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
