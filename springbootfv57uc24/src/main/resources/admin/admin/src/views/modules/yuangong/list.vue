<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px","margin":"0 auto","color":"#666","background":"#f5f5f5","width":"calc(100% - 60px)","fontSize":"14px","height":"100%"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"0px solid rgba(0, 0, 0, 0.05)","padding":"20px 20px","margin":"0px","overflow":"hidden","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"linear-gradient(323deg, rgba(221,221,221,.5) 0%, rgba(255,255,255,.5) 70%, rgba(221,221,221,.5) 100%),url(http://codegen.caihongy.cn/20230809/02ecb2d595ed42a2b0e8af76771b2f48.gif)","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","color":"#333","textAlign":"left","background":"none","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">技术部门员工工号</label>
						<el-input v-model="searchForm.yuangonggonghao" placeholder="技术部门员工工号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">技术部门员工姓名</label>
						<el-input v-model="searchForm.yuangongxingming" placeholder="技术部门员工姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0px","margin":"0px 0 0px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","fontSize":"inherit","order":"1"}'>
					...
					<el-button class="btn18" v-if="isAuth('yuangong','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="content" label="状态">
						<template slot-scope="scope">
                            <el-switch
								...
								@change="(e)=>yuangongstatusChange(e,scope.row)">
                            </el-switch>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yuangong','分配任务')" @click="renwufenpeiCrossAddOrUpdateHandler(scope.row,'cross','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								分配任务
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuangong','日程安排')" @click="richenganpaiCrossAddOrUpdateHandler(scope.row,'cross','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								日程安排
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuangong','技术部门员工工资')" @click="yuangonggongziCrossAddOrUpdateHandler(scope.row,'cross','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								技术部门员工工资
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuangong','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
		...
		<renwufenpei-cross-add-or-update v-if="renwufenpeiCrossAddOrUpdateFlag" :parent="this" ref="renwufenpeiCrossaddOrUpdate"></renwufenpei-cross-add-or-update>
		<richenganpai-cross-add-or-update v-if="richenganpaiCrossAddOrUpdateFlag" :parent="this" ref="richenganpaiCrossaddOrUpdate"></richenganpai-cross-add-or-update>
...
import renwufenpeiCrossAddOrUpdate from "../renwufenpei/add-or-update";
import richenganpaiCrossAddOrUpdate from "../richenganpai/add-or-update";
import yuangonggongziCrossAddOrUpdate from "../yuangonggongzi/add-or-update";
	export default {
		data() {
			return {
				...
				renwufenpeiCrossAddOrUpdateFlag: false,
				richenganpaiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			renwufenpeiCrossAddOrUpdate,
			richenganpaiCrossAddOrUpdate,
		...
		methods: {
			...
			renwufenpeiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				this.renwufenpeiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.renwufenpeiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.renwufenpeiCrossaddOrUpdate.init(row.id,type);
				});
			},
			richenganpaiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.richenganpaiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuangong');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.richenganpaiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.richenganpaiCrossaddOrUpdate.init(row.id,type);
				});
			},
			yuangonggongziCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yuangonggongziCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
    ...
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        ...
        this.$http({
          url: "yuangong/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
