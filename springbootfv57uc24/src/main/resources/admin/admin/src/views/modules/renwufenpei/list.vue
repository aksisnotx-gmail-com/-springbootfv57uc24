<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px","margin":"0 auto","color":"#666","background":"#f5f5f5","width":"calc(100% - 60px)","fontSize":"14px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid rgba(0, 0, 0, 0.05)","padding":"20px 20px","margin":"0px","overflow":"hidden","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"linear-gradient(323deg, rgba(221,221,221,.5) 0%, rgba(255,255,255,.5) 70%, rgba(221,221,221,.5) 100%),url(http://codegen.caihongy.cn/20230809/02ecb2d595ed42a2b0e8af76771b2f48.gif)","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0px","margin":"0px 0 0px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('renwufenpei','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('renwufenpei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"20px","boxShadow":"0 2px 3px 0px rgba(100,100,100,.05)","borderColor":"rgba(252,234,238,1)","margin":"0 0 0px","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230809/02ecb2d595ed42a2b0e8af76771b2f48.gif)","borderWidth":"0px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('renwufenpei','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('renwufenpei','工作汇报')" @click="gongzuohuibaoCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								工作汇报
							</el-button>
							<el-button class="edit" v-if=" isAuth('renwufenpei','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('renwufenpei','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<gongzuohuibao-cross-add-or-update v-if="gongzuohuibaoCrossAddOrUpdateFlag" :parent="this" ref="gongzuohuibaoCrossaddOrUpdate"></gongzuohuibao-cross-add-or-update>
...
import gongzuohuibaoCrossAddOrUpdate from "../gongzuohuibao/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				gongzuohuibaoCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			gongzuohuibaoCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.gongzuohuibaoCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','renwufenpei');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.gongzuohuibaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.gongzuohuibaoCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
          let urlwanchengriqi = 'renwufenpei/remind/wanchengriqi/2';
		 urlwanchengriqi+=`?remindend=0`,

            	urlwanchengriqi+=`&remindstart=-1`,
          ...
          this.$http({
            ...
            if (data && data.code == 0 && data.count>0 ) {
              this.$alert(`完成日期有${data.count}条数据到期`)
            }
          });
    },
    search() {
	...
	renwufenpeistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'renwufenpei/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
